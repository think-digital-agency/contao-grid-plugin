...
/*
 * Grey grid-class hint in the element list. The button_callback is wired via
 * #[AsCallback] on ContentPaletteListener.
 */
...
/*
 * Legacy columns this bundle no longer uses (push, pull, offset-right, block
 * grid, additional-classes — 0 rows on the demo, features dropped per ADR-14).
 * Kept SQL-only for rollback / cross-site verification, the same way WP-3 kept
 * rsce_data (ADR-4b); a dedicated bundle migration drops them once verified
...
/*
 * Wrapper start/stop palettes, unchanged from the legacy field set.
 */
